...
pub struct LengthPrefixedString<'s> {
  pub string: &'s str,
}

impl<'i> LengthPrefixedString<'i> {
  pub fn parse(mut input: &'i [u8]) -> IResult<&'i [u8], Self> {
    let ((input, _), len) = parse_len((input, 0)).map_err(|err| err.map_input(|(input, _)| input))?;
    dbg!(len);
    let (input, string) = map_res(take(len), str::from_utf8)(input)?;
...
mod tests {
  ...
  fn length_127() {
    ...
    assert_eq!(LengthPrefixedString::parse(&input), Ok(([].as_slice(), LengthPrefixedString { string: &string })));
  }
  #[test]
  fn length_16383() {
